...
folder_name2 = r'D:\FYJ\Choice大数据\市场关注度明细查询0330'
folder_name3 = r'D:\FYJ\Choice大数据\社区活跃度明细查询'

df_insert = pd.read_excel(r'D://FYJ//Choice大数据//市场关注截面数据//20210401截面数据.xls')
...
file_names1 = os.listdir(folder_name2)
for i, filename in enumerate(file_names1):
    if filename.endswith(".xls"):
        # print(df_insert[df_insert["代码"].isin([filename.split("-")[0]])]) # 输出指定列等于目标值的行
        print(filename.split("-")[0])
        df = pd.read_excel(folder_name2+'//'+filename)
        insertRow = df_insert[df_insert["代码"].isin([filename.split("-")[0]])]
        print(df_insert[df_insert["代码"].isin([filename.split("-")[0]])])
        new_df = pd.concat([insertRow,df],ignore_index = True)
        new_df.to_excel("D:\FYJ\Choice大数据\市场关注度明细查询0401\{}x".format(filename), engine='xlsxwriter')
    if filename.endswith(".xlsx"):
        print(filename.split("-")[0])
        df = pd.read_excel(folder_name2+'//'+filename, engine='openpyxl')
        insertRow = df_insert[df_insert["代码"].isin([filename.split("-")[0]])]
        print(df_insert[df_insert["代码"].isin([filename.split("-")[0]])])
        new_df = pd.concat([insertRow, df], ignore_index=True)
        new_df.to_excel("D:\FYJ\Choice大数据\市场关注度明细查询0401\{}".format(filename), engine='xlsxwriter')
